...
from eryn.moves.delayedrejection import DelayedRejection 
...
drmoves = DelayedRejection(moves, max_iter=20)
...
model = Model(
    ensemble.log_prob_fn,
    ensemble.compute_log_prob,
    ensemble.compute_log_prior,
    ensemble.temperature_control,
    map,
    ensemble._random,
) 

# I need to also input the initial logprior. This is quite shitty but it works
state = State(coords, log_prob=log_prob, betas=betas, blobs=blobs, inds=inds)
coords_for_prior = {name: branch.coords for name, branch in state.branches.items()}
logp  = model.compute_log_prior_fn(coords_for_prior, inds=state.branches_inds)
state = State(coords, log_prob=log_prob, log_prior=logp, betas=betas, blobs=blobs, inds=inds)
